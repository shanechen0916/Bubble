(function () {
    ...
    let optsDevice = document.getElementById('opts-device');
    var btnPause = document.getElementById('btn-pause');
    var btnStep = document.getElementById('btn-step');
    let optsDebugMode = document.getElementById('opts-debug-mode');
    let btnShowFPS = document.getElementById('btn-show-fps');
    let inputSetFPS = document.getElementById('input-set-fps');
    let btnRecompile = document.getElementById('btn-recompile');
    ...
    function setCookie(name, value, days) {
        ...
        document.cookie =
            name + '=' + encodeURIComponent(value) + ';expires=' + expires.toGMTString();
    }
    ...
    function getEmulatedScreenSize() {
        let w, h;
        let idx = optsDevice.value;
        if (idx === '0') {
            w = designWidth;
            h = designHeight;
        } else {
            let info = devices[parseInt(idx) - 1];
            w = info.width;
            h = info.height;
        }
        return {
    ...
    function showSplash() {
        let LOGO_IMG_L_W = 416;
        let LOGO_IMG_L_H = 87;
        let LOGO_SIZE = 0.4;

        let size = isFullScreen()
        ...
        if (size.width < size.height) {
            ...
            let logoDisplayH = ((size.width * LOGO_SIZE) / LOGO_IMG_L_W) * LOGO_IMG_L_H;
    ...
    function initPreviewOptions() {
        let defaultDevice = getCookie('device');
        let defaultRotate = getCookie('rotate');

        let hasDefaultDevice = defaultDevice !== null;
        let hasDefaultRotate = defaultRotate !== null;
    ...
    function onload() {
        ...
        inputSetFPS.addEventListener('change', function (event) {
            let fps = parseInt(inputSetFPS.value);
            if (isNaN(fps)) {
                fps = 60;
        ...
        let onStart = function () {
            ...
            cc.assetManager.loadAny(
                ...
                function (finish, totalCount) {
                    let percent = (100 * finish) / totalCount;
                    if (progressBar) {
